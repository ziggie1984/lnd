...
func (d *AuthenticatedGossiper) handleChanUpdate(nMsg *networkMsg,
	...
	if err := d.cfg.Router.UpdateEdge(update, ops...); err != nil {
		if routing.IsError(
			...
			// This case is necessary to prevent a race where the
			// peer sends us a ChanUpdate but we are in the process
			// of removing/adding the edge to our db (for scid alias
			// channels). This leads to the case where we rate limit
			// the ChanUpdate from the peer never receiving the
			// ChanUpdate message with the __dont_forward__ bit
			// unset.
			log.Debugf("Update edge for short_chan_id(%v) got: %v",
				shortChanID, err)
		} else {

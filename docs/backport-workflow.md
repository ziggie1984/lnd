...
Both scenarios work identically. The second scenario is common when backporting is decided after the fact.
...
4. Fix the label, workflow will retry on next trigger
...
3. **Manual backport:**
   - If automated backport fails repeatedly
   - Follow manual backport instructions in [code_contribution_guidelines.md](code_contribution_guidelines.md#manual-backporting)

...
func TestChannelMaxFeeRate(t *testing.T) {
	...
	// We cap the feerate at the old feerate in case we would go lower
	// because of the fee allocation
...
func TestIdealCommitFeeRate(t *testing.T) {
	...
	maxFeeRate := func(c *LightningChannel,
		...
		balance, weight := c.availableBalance()
		feeRate := c.localCommitChain.tip().feePerKw
		oldFee := feeRate.FeeForWeight(weight)

		maxBalance := balance.ToSatoshis() + oldFee
		expectedFee := math.Max(
			float64(maxBalance)*maxFeeAlloc, float64(oldFee),
		)

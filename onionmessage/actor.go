...
const (
	...
	// 1000 bits/s). At ~32 KB per spec-max onion packet this is roughly
	// two max-size messages per second per peer. A value of zero
	// disables the per-peer limiter entirely.
	DefaultPeerOnionMsgKbps = 512

	// DefaultPeerOnionMsgBurstBytes is the default per-peer token bucket
	// depth, in bytes. Sized to hold approximately eight spec-max onion
	// messages so a peer can briefly burst above the sustained rate
	// without drops.
	...
	// depth, in bytes. Sized to hold approximately fifty spec-max onion
	// messages.

...
// TestPeerRateLimiterRetainsBucketAcrossDrain verifies that a peer whose
// bucket has been drained stays drained: the registry never resets a
// per-peer bucket, so once a peer exhausts its allowance the next AllowN
// call still rejects until time elapses. This is the load-bearing
// invariant that prevents a hostile peer from cycling its connection to
// reset its burst allowance.
func TestPeerRateLimiterRetainsBucketAcrossDrain(t *testing.T) {
	t.Parallel()

	const burstMessages = 2
	// A very low refill rate so the bucket does not naturally refill
	// inside the test window. This isolates the retention behavior from
	// any time-based recovery.
	p := NewPeerRateLimiter(1, burstMessages*msgBytes)
	...
	// Repeatedly attempting more messages must keep failing — the
	// bucket is retained, not reset.
	for i := 0; i < 5; i++ {
		require.False(t, p.AllowN(peer, msgBytes),
			"drained bucket must stay drained")
	}
	require.Equal(t, 1, peerMapLen(p),
		"registry size must not change across drained reads")
}
...
// TestFirstGlobalDropClaimNoopLimiter verifies that FirstGlobalDropClaim
// returns false for a noop (disabled) limiter, since a disabled limiter
// never produces drops and must not claim the first-drop flag.
func TestFirstGlobalDropClaimNoopLimiter(t *testing.T) {
	t.Parallel()

	lim := NewGlobalLimiter(0, 0)
	require.False(t, FirstGlobalDropClaim(lim))
}
